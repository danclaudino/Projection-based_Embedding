...
class Embed:
    """Class to provide the pieces for projection-based embedding calculations."""
    ...
    def spade_partition(self, sigma):
        """Defines the orbital partition for the occupied space.
    ...
    def open_shell_subsystem(self, Ca, Cb):
        ...
        # Energy
        E = matrix_dot(self.wfn.H().np, Da) + matrix_dot(self.wfn.H().np, Db) +\
            0.5*(matrix_dot(Ja, Da) + matrix_dot(Jb, Db) + matrix_dot(Ja, Db) + matrix_dot(Jb, Da) -\
            self.wfn.functional().x_alpha()*(matrix_dot(Ka, Da) + matrix_dot(Kb, Db))) + E_xc

        return E, E_xc, Ja, Jb, Ka, Kb, Va, Vb


    def subsystem(self, C):
        """
    ...
    def orthonormalize(self, S, C, n_non_zero):
        """ Orthonormalizes a set of orbitals (vectors).
